...
export class MultiLineComponent implements OnChanges {
  ...
  ngOnChanges() {
    ...
    this.data = dataGot.map((v) => v.values.map((v) => v.time))[0];
  ...
  initChart(): void {
    ...
    this.x.domain([d3Array.min([xmin]), d3Array.max([xmax])]);
    let ymin = 0;
    let ymax = 0;
    for (let key in this.ys) {
    ...
    this.y.domain([d3Array.min([ymin]), d3Array.max([ymax])]);
