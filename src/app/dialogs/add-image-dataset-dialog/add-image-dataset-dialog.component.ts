...
export class AddImageDatasetDialogComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.datasetToSee = this.datasetFactory.createDummyFromImageCsv(this.images, "id");
    this.dataSource = this.datasetViewService.createViewData(this.datasetToSee , 10);
      ...
      this.featureApi.postEntity(feat).subscribe(
        (feature:Feature)=>{
          ...
          if(i === feat_length){
            ...
            this.datasetApi.uploadNewDataset(this.datasetToSee).subscribe((dataset_posted)=>{
